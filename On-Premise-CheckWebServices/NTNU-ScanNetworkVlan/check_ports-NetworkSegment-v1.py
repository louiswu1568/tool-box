...
# IP 列表文件和輸出文件的位置
ip_file_path = "/home/kali/Sys/ScanNetwork_Vlan-v1.txt"  # CIDR 或 IP 地址的清單
output_file_path = "/home/kali/Sys/ScanNetwork_Vlan-final-result.xlsx"
...
with open(ip_file_path, "r") as ip_file, ThreadPoolExecutor(max_workers=10) as executor:
    ...
    for line in ip_file:
        ...
        if line:  # 忽略空行
            try:
                # 使用 ipaddress 模組解析網段並生成 IP 地址列表
                network = ipaddress.ip_network(line, strict=False)
                for ip in network.hosts():
                    futures.append(executor.submit(check_ip, str(ip)))
            except ValueError:
